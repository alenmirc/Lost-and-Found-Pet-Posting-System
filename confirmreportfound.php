...
$petid = $_POST['petid'];
$contactname = $_POST['foundername'];
$contactemail = $_POST['founderemail'];
$contactnumber = $_POST['foundercontact'];
...
if(!isset($_SESSION['users'])){
  ...
  if(!isset($_SESSION['fillup'])) {
    header("Location: reportfoundpet.php");
  }
...
<?php
$sql = "SELECT id, pet, petid, breed, fur, petname, petimage, date FROM registerpet WHERE petid='$petid'";
$result = $db->query($sql);
...
?>
 <div class="col-md-12">
 <label class="label">Pet ID: </strong><?php echo $row['petid'] ?></label>
        <br>
</div>
 <div class="col-md-12">
...
<label class="label">Place Found: </strong><?php echo $placefound;?></label>
        <br>
        <br>
...
<label class="label">Did you surrender the pet to authorities?: </strong> <?php if ($surrender==1){ echo 'Yes'; } else { echo 'No'; } ?></label>
        <br><br>
</div>
 <div class="col-md-12">
<label class="label">Authority Name: </strong><?php echo $authorityname;?></label>
        <br>
</div> <div class="col-md-12">
<label class="label">Authority Contact Number: </strong><?php echo $authoritycontact;?></label>
        <br>
</div> <div class="col-md-12">
<label class="label">Authority Address: </strong><?php echo $authorityaddress;?></label>
        <br>
</div> 
<br><br><br>
<div class="col-md-12">
        ...
        <input type='hidden' name='petid' value='<?php echo $row['petid']?>'>
          ...
          <input type='hidden' name='foundername' value='<?php echo $contactname;?>'>
          <input type='hidden' name='placefound' value='<?php echo $placefound;?>'>
          <input type='hidden' name='foundercontact' value='<?php echo $contactnumber;?>'>
          <input type='hidden' name='founderemail' value='<?php echo $contactemail;?>'>
          <input type='hidden' name='registered' value='1'>
          <input type='hidden' name='surrender' value='<?php echo $surrender;?>'>
          <input type='hidden' name='authorityname' value='<?php echo $authorityname;?>'>
          <input type='hidden' name='authorityaddress' value='<?php echo $authorityaddress;?>'>
          <input type='hidden' name='authoritycontact' value='<?php echo $authoritycontact;?>'>
          <?php }} else {
   $_SESSION['notfound'] = "Pet ID not found in database, please try again!";
   header("Location: reportfoundregistered.php");
        }
?>      
                        <input type="submit" name="confirmfound" value="Confirm" class="btn btn-primary">
                ...
                <a href="reportfoundpet.php"><button type='submit' name='cancel' class='btn btn-danger'>Cancel</button></a>
              </div>
            </div>
