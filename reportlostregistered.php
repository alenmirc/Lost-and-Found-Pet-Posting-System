          ...
          <div class="row no-gutters">
            <div class="col-md-7">
              <div class="contact-wrap w-100 p-md-5 p-4">
                ...
                <form method="POST" action="confirmreportlost.php" enctype="multipart/form-data">
                  <div class="row">
                  <div class="col-md-12">
                    <P class="mt-3">Registered Pet Information</P>
</div>
                    
                    <div class="col-md-12">
                      <div class="form-group">
                      <label for="selectPet">Select Pet:</label>
                        <select class="form-control" name="petselect" id="selectPet" required>
                        <?php

$userid = $users['id'];
$sql = "SELECT id, petid, petname, petimage, date FROM registerpet WHERE userid=$userid ORDER BY id DESC";
$result2 = $db->query($sql);
if ($result2->num_rows > 0) {
  // Output data of each row
  while($row = $result2->fetch_assoc()) {
?>
                            <option value="<?php echo $row['id'] ?>"><?php echo $row['petname'] ?></option>
             <?php }} ?>
                            </select>
                      </div>
                    </div>
                    <div class="col-md-12">
                    <div class="form-group">
                        <label class="label" for="ownersname">Last Seen</label>
                        <input type="text" class="form-control"name="lastfound" placeholder="Last Seen" required>
                      </div>
                    </div>
