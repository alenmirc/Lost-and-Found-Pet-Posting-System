...
?>
    ...
    <section class="hero-wrap hero-wrap-2" style="background-image: url('images/bgfound.jpg');" data-stellar-background-ratio="0.5">
  <div class="overlay"></div>
  <div class="container">
  <div class="row no-gutters slider-text align-items-center" data-scrollax-parent="true">
    <div class="col-md-12 ftco-animate text-center">
          	<h1 class="mb-4">You’ve found a pet. We can help.</h1>
            <p><a href="reportfoundpet.php" class="btn btn-primary mr-md-4 py-3 px-4">Report a Found Pet <span class="ion-ios-arrow-forward"></span></a></p>
          </div>
...
if (isset($_GET['search'])) {
  ...
  $sql = "SELECT id, registered, petname, petid, pet, fur, breed, petimage, placefound, date, approved, expirationdate FROM reportfoundpet WHERE expirationdate >= '$currentDate' AND approved = 1 AND (petname LIKE '%$searchTerm%' OR petid LIKE '%$searchTerm%' OR pet LIKE '%$searchTerm%' OR fur LIKE '%$searchTerm%' OR breed LIKE '%$searchTerm%' OR placefound LIKE '%$searchTerm%') ORDER BY id DESC";
} else {
  // Default SQL query without search term but with approved condition
  $sql = "SELECT id, registered, petname, petid, pet, fur, breed, petimage, placefound, date, approved, expirationdate FROM reportfoundpet WHERE expirationdate >= '$currentDate' AND approved = 1 ORDER BY id DESC";
}
