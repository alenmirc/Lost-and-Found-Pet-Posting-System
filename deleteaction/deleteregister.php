...
$sql = "SELECT id, petid, userid FROM registerpet WHERE petid='$deleteid'";
$result = $db->query($sql);
...
if ($row['petid'] === $deleteid && $row['userid'] !== $users['id']) {
    header('Location: ../profile.php');
    exit(); 
}
else {
    $sql="DELETE FROM registerpet WHERE petid='$deleteid'";
    $db->query($sql);
